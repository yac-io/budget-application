...
RSpec.describe InvestmentsController, type: :controller do
  ...
  let(:valid_attributes) {
    build(:investment).attributes
  }

  let(:invalid_attributes) {
    build(:investment, isin: '').attributes
  }
  ...
  describe 'POST #create' do
    context 'with valid params' do
      it 'creates a new Investment' do
        account = create(:account, user: @user)
        expect {
          post :create, params: {investment: valid_attributes, account_id: account.id}, session: valid_session
        }.to change(Investment, :count).by(1)
      end

      it 'redirects to the account investments' do
        account = create(:account, user: @user)
        post :create, params: {investment: valid_attributes, account_id: account.id}, session: valid_session
        expect(response).to redirect_to(Investment.last.account)
      end
    end
  ...
  describe 'PUT #update' do
    context 'with valid params' do
      ...
      it 'updates the requested investment' do
        valid_attributes[:user_id] = @user.id
        investment = Investment.create! valid_attributes
        put :update, params: {id: investment.to_param, investment: new_attributes, account_id: investment.account.id}, session: valid_session
        investment.reload
        expect(investment.name).to eq('new_name')
      end

      it 'redirects to the investment account' do
        valid_attributes[:user_id] = @user.id
        investment = Investment.create! valid_attributes
        put :update, params: {id: investment.to_param, investment: valid_attributes, account_id: investment.account.id}, session: valid_session
        expect(response).to redirect_to(investment.account)
      end
    end
  ...
  describe 'DELETE #destroy' do
    it 'destroys the requested investment' do
      ...
      investment = Investment.create! valid_attributes
      expect {
        delete :destroy, params: {id: investment.to_param, account_id: investment.account.id}, session: valid_session
      }.to change(Investment, :count).by(-1)
    end
